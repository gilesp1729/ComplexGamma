...
// if TRUE, destroying TK window takes whole app down
static BOOL tkQuitApp = FALSE;

// Background is white or black
static BOOL tkWhiteBackground = FALSE;
...
void tkExec(void)
{
    ...
    // GP - remove this crap and use a timer to time the redraws,
    // so the caller's message loop processes everything.
#if 1
    while (GL_TRUE)
    {
        /*
...
void tkMouseWheelFunc(GLenum(CALLBACK *Func)(int, int, int))
{
    MouseWheelFunc = Func;
}
...
GLenum tkInitWindow(char *title, BOOL white_bgnd, HMENU menu, BOOL quit_app)
{
    ...
    return tkInitWindowAW(title, FALSE, white_bgnd, menu, quit_app);
}

GLenum tkInitWindowAW(char *title, BOOL bUnicode, BOOL white_bgnd, HMENU menu, BOOL quit_app)
{
    WNDCLASS wndclass;
    RECT     WinRect;
    HANDLE   hInstance;
...
#if 0 // GetVersionInfo is deprecated
    ...
    wndclass.hbrBackground = GetStockObject(tkWhiteBackground ? WHITE_BRUSH : BLACK_BRUSH);
    ...
    // GP - if we are re-registering, that's ok - tolerate failure
#if 0
    /*
...
PIXELFORMATDESCRIPTOR pfd;
   ...
   switch (message) 
      ...
      case WM_LBUTTONDOWN:
         ...
         if (MouseDownFunc)
            {
             GLenum mask = TK_LEFTBUTTON;
             if (wParam & MK_SHIFT)
                 mask |= TK_SHIFT;
             if (wParam & MK_CONTROL)
                 mask |= TK_CONTROL;
             if ((*MouseDownFunc)(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), mask) )
               {
               ForceRedraw( hWnd );
               }
            }
      ...
      case WM_MOUSEWHEEL:
          if (MouseWheelFunc)
          {
              if ((*MouseWheelFunc)(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam),
...
// array of char *, gaszSysClrNames.
    ...
    for (i = 0; i < nColors; i++)
    {
        ...
        if ( lRet == ERROR_SUCCESS && dwDataType == REG_SZ )
        {
            ...
            sscanf_s(achColor, "%ld %ld %ld", &r, &g, &b);
            pcr[i] = RGB(r, g, b);
        }
    }
...
PrintMessage( const char *Format, ... )
{
    ...
    vsprintf_s(Buffer, 256, Format, ArgList);
    va_end(ArgList);
